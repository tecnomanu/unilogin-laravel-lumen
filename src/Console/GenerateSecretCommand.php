...
/*
 * This file is part of unilogin-auth.
 *
...
class GenerateSecretCommand extends Command
{
     ...
     * @var string
     */
    protected $signature = 'unilogin:create-secret
    ...
    public function handle()
    {
        ...
        if (Str::contains(file_get_contents($path), 'UNILOGIN_SECRET') === false) {
            // create new entry
            file_put_contents($path, PHP_EOL."UNILOGIN_SECRET=$key".PHP_EOL, FILE_APPEND);
        } else {
            ...
            file_put_contents($path, str_replace(
                'UNILOGIN_SECRET='.$this->laravel['config']['unilogin.secret'],
                'UNILOGIN_SECRET='.$key, file_get_contents($path)
            ));
        }
    ...
    {
        $this->laravel['config']['unilogin.secret'] = $key;

        $this->info("unilogin-auth secret [$key] set successfully.");
    }
